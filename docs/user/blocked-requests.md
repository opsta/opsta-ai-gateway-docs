...
::: info Prerequisite
You're signed in ([Get access](/user/get-access)).
:::
...
[Usage & budget](/user/usage-and-budget). The [request lifecycle](/overview/request-lifecycle) table maps each
status to its gate.
:::

## Next steps

- Administrators tune rules in [Guardrails](/admin/guardrails).
