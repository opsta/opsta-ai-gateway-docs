...
[Audit log](/admin/audit-log) page for the console workflow.
...
- **Access control** — least-privilege [RBAC](/security/rbac) with verified identity.
- **Accountability** — this audit trail, including denials.
- **Data residency** — everything stays in your cluster ([Data sovereignty](/security/data-sovereignty)).
- **Encryption in transit** — TLS terminated in-cluster ([TLS & domains](/operate/tls-and-domains)).
- **Recoverability** — database backups and DR ([Backup & DR](/operate/backup-and-dr)).
- **Supply-chain assurance** — pinned, scanned images ([Hardening](/security/hardening)).
...
- [Audit log](/admin/audit-log) — read and filter the trail in the console.
- [RBAC model](/security/rbac) — the roles whose actions are audited.
- [Hardening](/security/hardening) — the controls behind the trail.
