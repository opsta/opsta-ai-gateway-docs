...
- **PostgreSQL** runs as a 3-instance CloudNativePG cluster with streaming replication and automatic failover.
  Pair this with [Backup & DR](/operate/backup-and-dr).
...
- **Observability** uses an **object-storage** backend (`observability.storage: object`) in HA rather than local
  volumes — see [Platform observability](/operate/observability-platform).

## Sizing

HA roughly doubles to triples the standalone footprint. Confirm your cluster has the headroom from
[Requirements](/operate/requirements) before enabling it, and remember that turning on the semantic features adds
Qdrant and Ollama on top.

## Next steps

- [Backup & DR](/operate/backup-and-dr) — protect the PostgreSQL source of truth.
- [Platform observability](/operate/observability-platform) — storage backend and retention in HA.
- [Upgrades](/operate/upgrades) — rolling upgrades with disruption budgets.
